...
def mapapp(request):
    if request.method == 'GET':
        ...
        context = {
            'message':message,
            'GMAPS_API_KEY': 'AIzaSyA2IbfRXEezBDndK3gHUy1RIXxxbtrBuNo'
        }

    if request.method == 'POST':

        osmPath = "../addismap/mapvis/oak.osm"
...
#####
        ...
        if len(shortestPath)==0:
            request.session['message'] = "Sorry Couldn't Find Path. Please try different nodes. The Open Street Map data for  isn't completely filled with all the nodes as accurately ."
            return redirect('/')
        else:
